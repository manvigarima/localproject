...
$al=".";
	...
	$image=new image();
	$totalEntries=$image->totalNoOfimages($al,$_SESSION['school_id']);
	...
	$rec = $image->display_image($page,$order,$al,$_SESSION['school_id']);	
/*	echo"<pre>";
	print_r($rec);
*/	
...
<script language="javascript">
	...
	{
	   //alert("hi");
	   var num=document.image.elements.length;
	   for(i=0;i<num;i++)
	   {
			tmp=document.image.elements[i];
	...
	function send_select()
	{
		...
		//alert("alert");
		document.image.send.value=value12;
	...
	function doselect1()
	{
		...
		if(!confirm('Are you sure you want to delete the Image?\n- to Delete the Image, hit OK\n- otherwise, hit Cancel'))
			return false;
			
		document.image.delet.value=val12;
		document.image.submit();
		return true;
	}
	 function export1()
	{
		...
		location.href='export_images.php?exprt='+val12;
		return true;
		...
		<div id="content_wrapper" style="margin-right:30px;"> 
			...
			<div style="border:1px solid #ccc;"  > 
				...
				<div class="content nomargin"  > 
                  ...
                  <form name="image" method="post" action="action.php?al=<? echo $_REQUEST['al'];?>&page=<? echo $page; ?>">
						...
						<thead>
						 ...
						 <tr><td width="100%" colspan="5"><b><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></b></td></tr>
											<tr>
												...
												?>     
                                                <input type="button" name="sub1" value="Add images" class="button_light"   onclick="return addimages()" />
									               <input type="submit" name="sub3" value="Delete images" class="button_light" onclick="doselect1(); return false;" />
												...
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
                  									  ...
                  									  <tr class="row_1">
										                    ...
										                    <th width="30%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==0){?>1<?php }else{?>0<?php }?>" class="tablehead"><span class="vote">Image</span></a>&nbsp;<?php if($order==0) {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
									                    <th width="30%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==2){?>3<?php }else{?>2<?php }?>" class="tablehead"> Link</a>&nbsp;<?php if($order=='2') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '3'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>	
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['scroll_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="edit.php?id=<?php echo $row['scroll_id'];?>">
														    <?   $x="../../../images/".$row['scroll_image'];?> </a><a href="edit.php?id=<?php echo $row['scroll_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']; }?>">
                   							 <img class="image_gap" src="<? echo $x; ?>"  width="100" height="60" alt="" border="0" />
                                                          
                                                          
														  </a></td>
										                  <td align="left"><a href="edit.php?id=<?php echo $row['scroll_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']; }?>"><?php echo $row['link']; ?></a></td>
										                
										                  <td align="left">
                                                         
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['scroll_id']?>&id=<?=$row['scroll_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?php 
														  if($row['status']==1)
														  echo"Active";
														  else
														  echo"Inactive";
														  ?></a>
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id=<?=$row['scroll_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"></a></td> 
									                  </tr>
													  <?php }
													  		else if($val == '1')
															{
													  ...
													  <tr class="row_color">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['scroll_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="edit.php?id=<?php echo $row['scroll_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']; }?>">
														    <?  $x="../../../images/".$row['scroll_image'];?>
														 <img class="image_gap" src="<? echo $x; ?>"  width="100" height="60" alt="" border="0" /></a></td>
										                  <td align="left"><a href="edit.php?id=<?php echo $row['scroll_id'];?>&page=<?php echo $page; ?>&&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al'] ;}?>"><?php echo $row['link']; ?></a></td>
										                  
										                  <td align="left">
                                     <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['scroll_id']?>&id=<?=$row['scroll_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"><?php 
														  if($row['status']==1)
														  echo"Active";
														  else
														  echo"Inactive";
