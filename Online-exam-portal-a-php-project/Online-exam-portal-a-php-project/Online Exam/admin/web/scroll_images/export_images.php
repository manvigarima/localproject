...
//echo $sql;
	...
	if(isset($_REQUEST['exprt']) && !empty($_REQUEST['exprt']))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$sql = "SELECT * FROM scroll_image WHERE scroll_id='".$val[$i]."' and school_id=".$_SESSION['school_id']."";
			$users=$objSql1->executeSql($sql);
		}
	}
    else
	{
		$sql = "SELECT * FROM scroll_image where school_id=".$_SESSION['school_id']."";
		$users=$objSql1->executeSql($sql);
	}	
	...
	if(is_array($users))
		{
				...
				$filename ="images_report".time().".doc";
				$title = "";
				...
				<tr  bgcolor='#72ACF3'>
				<td>Search Criteria: Scroll Images</td> 
				<td>Total Records: ".count($users)."</td></tr>
				<tr><td colspan='2' align='center' style='color:#336699; font-size:16px;'><strong>Scroll Images</strong></td></tr>";
				...
				for($i=0;$i<count($users);$i++)
				{
					...
					<tr><td style='font-size:14px; color:#009966; font-weight:bold;'>Link:".ucfirst($users[$i]['link'])." </td></tr>
					<tr><td><strong>Status : ";
					if($users[$i]['status']==1){
						$contents.='Active';
					}
					...
					<tr><td style='padding-left:5px;'><img src=".URL.'images/'.$users[$i]['scroll_image']." height='80' width='80'> </td></tr>
					<tr><td>&nbsp;</td></tr>";
