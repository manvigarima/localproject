<?php 
	...
	if(!empty($_REQUEST['al']))
	{
	 ...
	 $totalEntries=$blog->totalNoOfBlogcomms($_GET['id']);
	...
	$rec = $blog->display_subforum_comms($page,$al,$order,$_GET['id']);
/*	echo"<pre>";
	print_r($rec);*/
											...
											<tr>
												<td colspan="5">
													<table width="100%" border="0" cellpadding="3" cellspacing="1" class="tborder">
									                      ...
									                      <th width="24%" align="left" ><a href="index.php?order=<?php if(empty($_GET['order'])){?>1<?php }elseif($_GET['order'] == '1'){?>0<?php }?>" class="tablehead">Blog Review</a>&nbsp;<?php if($_GET["order"]=='0') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($_GET["order"] == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="center" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['b_r_id'];?>" value="642" class="input2" /></td>
														  <td align="left">
                                          <?php echo ucfirst($row['review_desc']);?></td>
                                                        ...
                                                        <a href="review_action.php?id_1=<?=$row['b_r_id']?>&state=<?=$row['status']?>&page=<?=$_GET['page']?>&id=<?=$_GET['id']?>">
                                                          ...
                                                          <td>
                                                            <a href="view_blog_review.php?id=<?=$row['b_r_id']?>&page=<?=$_GET['page']?>">
													  ...
													  <tr class="row_color">
										                  <td height="24" align="center" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['b_r_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><?php echo ucfirst($row['review_desc']);?></td>
                                                           <td align="left"><?=$row['create_date'] ?></td>
