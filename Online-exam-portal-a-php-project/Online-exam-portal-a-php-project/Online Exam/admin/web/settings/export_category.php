...
//echo $sql;
	...
	if(isset($_REQUEST['exprt']) && !empty($_REQUEST['exprt']))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$sql = "SELECT * FROM category WHERE cat_id='".$val[$i]."'";
	...
	if(is_array($users))
		{
				...
				$filename ="category_report".time().".doc";
				$title = "";
				...
				<tr  bgcolor='#72ACF3'>
				<td>Search Criteria: Categories</td> 
				<td>Total Records: ".count($users)."</td></tr>
				<tr><td colspan='2' align='center' style='color:#336699; font-size:16px;'><strong>Categories</strong></td></tr>";
				...
				for($i=0;$i<count($users);$i++)
				{
					...
					<tr><td style='font-size:14px; color:#009966; font-weight:bold;'>Category Name:".ucfirst($users[$i]['cat_name'])." </td></tr>
					<tr><td><strong>Status : ";
