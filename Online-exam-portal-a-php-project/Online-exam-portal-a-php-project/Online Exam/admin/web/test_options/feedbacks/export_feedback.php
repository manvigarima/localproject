...
//echo $sql;
	...
	if(isset($_REQUEST['exprt']) && !empty($_REQUEST['exprt']))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$sql = "SELECT * FROM school_feedback WHERE feedback_id ='".$val[$i]."' and school_id=".$_SESSION['school_id']."";
			$users=$objSql1->executeSql($sql);
		}
	}
    else
	{
		$sql = "SELECT * FROM school_feedback where school_id=".$_SESSION['school_id']."";
		$users=$objSql1->executeSql($sql);
	}	
	...
	if(is_array($users))
		{
				...
				<tr><td colspan='2' align='center' style='color:#336699; font-size:16px;'><strong>Feedback</strong></td></tr>";
				...
				for($i=0;$i<count($users);$i++)
				{
					...
					<tr><td style=' font-weight:bold;'><strong>UserName:</strong>".ucfirst($users[$i]['feedback_name'])." </td></tr>
					<tr><td><strong>Status : ";
						$contents.=ucfirst($users[$i]['status']);
						$contents.= "</strong> </td></tr>
					<tr><td style='padding-left:5px;'><strong> FeedBack:</strong>".$users[$i]['comment']."</td></tr>
