...
$al=".";
	...
	$feedback1=new Tefeedback();
	 $totalEntries=$feedback1->totalNoOfTefeedback1($al,$_SESSION['school_id']);
	...
	$rec = $feedback1->display_tefeedback1($page,$order,$al,$_SESSION['school_id']);	
	/*echo"<pre>";
	print_r($rec);*/
...
<body>
    ...
    <?php  include '../../left.php'; ?>
		...
		<ul class="first_level_tab"> 
				<li> 
					<a href="<?php echo $admin_path;?>test_options/feedbacks/index.php" class="active"> 
						Manage Feedback
					</a> 
				</li>
						 ...
						 <tr><td width="100%" colspan="5"><b><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></b></td></tr>
											<tr>
                                               ...
                                               <!-- <input type="button" name="sub1" value="Add feedback" class="button"   onclick="return addfeedback()" />-->
									               <input type="submit" name="sub3" value="Delete feedback" class="button_light" onclick="doselect1(); return false;" />
											...
											<tr>
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['feedback_id'];?>" value="642" class="input2" /></td>
														  <td align="left">
														    <?=$row['feedback_name']?>
                                                          ...
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['feedback_id']?>&id=<?=$row['feedback_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?php 
														  ...
														  echo"Inactive";
														  ?></a>
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id=<?php echo $row['feedback_id ']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"></a></td> 
									                  </tr>
													  <?php }
													  		else if($val == '1')
															{
													  ...
													  <tr class="row_color">
										                  ...
										                  <td align="left"><?=$row['comment'];?></td>
										                  <td align="left">
                                     <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['feedback_id']?>&id=<?=$row['feedback_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"><?php 
														  if($row['status']=="active")
														  echo"Active";
														  else
														  echo"Inactive";
