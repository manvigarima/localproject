...
//echo $sql;
	...
	if(isset($_REQUEST['exprt']) && !empty($_REQUEST['exprt']))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$sql = "SELECT * FROM videos WHERE video_id ='".$val[$i]."' and school_id=".$_SESSION['school_id']."";
			$users=$objSql1->executeSql($sql);
		}
	}
    else
	{
		$sql = "SELECT * FROM videos where school_id=".$_SESSION['school_id']." ";
		$users=$objSql1->executeSql($sql);
	}	
	...
	if(is_array($users))
		{
				...
				$filename ="videos_report".time().".doc";
				$title = "";
				...
				<tr  bgcolor='#72ACF3'>
				<td>Search Criteria: Videos</td> 
				<td>Total Records: ".count($users)."</td></tr>
				<tr><td colspan='2' align='center' style='color:#336699; font-size:16px;'><strong>Videos</strong></td></tr>";
				...
				for($i=0;$i<count($users);$i++)
				{
					...
					<tr><td style=' font-weight:bold;'><strong>Video title:</strong>".ucfirst($arr['video_title'])." </td></tr>
					<tr><td><strong>Status : ";
						$contents.=ucfirst($users[$i]['status']);
						$contents.= "</strong> </td></tr>
					<tr><td style='padding-left:5px;'><strong> Video description:</strong>".$users[$i]['video_desc']."</td></tr>
					<tr><td style='padding-left:5px;'><strong>Create Date :</strong>".$users[$i]['create_date']."</td></tr>
					<tr><td style='padding-left:5px;'><strong>Video Image :</strong><img src=".URL.'uploads/vimages/'.$users[$i]['video_image']." height='80' width='80'></td></tr>	<tr><td style='padding-left:5px;'><strong>YouTube URL :</strong>".$users[$i]['youtube_url']."</td></tr><tr><td>&nbsp;</td></tr>";
