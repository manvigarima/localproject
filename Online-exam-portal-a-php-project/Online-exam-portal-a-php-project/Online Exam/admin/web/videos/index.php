...
$al=".";
	...
	$video=new video();
	$totalEntries=$video->totalNoOfvideo($al,$_SESSION['school_id']);
	...
	$rec = $video->display_video($page,$order,$al,$_SESSION['school_id']);	
/*	echo"<pre>";
	print_r($rec);
*/	
...
<script language="javascript">
	...
	{
	   //alert("hi");
	   var num=document.video.elements.length;
	   for(i=0;i<num;i++)
	   {
			tmp=document.video.elements[i];
	...
	function send_select()
	{
		...
		//alert("alert");
		document.video.send.value=value12;
	...
	function doselect1()
	{
		...
		document.video.delet.value=val12;
		document.video.submit();
		return true;
	}
	function addvideo()
	{
	if(!confirm('Are you sure you want to add the video?\n- to Add the video, hit OK\n- otherwise, hit Cancel'))
	return false;
	 ...
	 function export1()
	{
		...
		location.href='export_videos.php?exprt='+val12;
		return true;
		...
		<div id="content_wrapper" style="margin-right:30px;"> 
			...
			<div style="border:1px solid #ccc;"  > 
				...
				<div class="content nomargin"  > 
                  ...
                  <form name="video" method="post" action="action.php?al=<? echo $_REQUEST['al'];?>&page=<? echo $page; ?>">
						...
						<thead>
						 ...
						 <tr><td width="100%" colspan="5"><b><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></b></td></tr>
											<tr>
												...
												?>     
                                             <input type="button" name="sub1" value="Add video" class="button_light"   onclick="return addvideo()" />
									               <input type="submit" name="sub3" value="Delete video" class="button_light" onclick="doselect1(); return false;" />
												...
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
                  									  ...
                  									  <tr class="row_1">
										                    ...
										                    <th width="30%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==0){?>1<?php }else{?>0<?php }?>" class="tablehead"><span class="vote">  Video Title  </span></a>&nbsp;<?php if($order==0) {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
									                    <th width="30%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==2){?>3<?php }else{?>2<?php }?>" class="tablehead">Video Image</a>&nbsp;<?php if($order=='2') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '3'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>	
									                    <th width="20%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==4){?>5<?php }else{?>4<?php }?>" class="tablehead"><strong>Youtube Url</strong></a>&nbsp;<?php if($order == '4') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '5'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th> 
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['video_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="edit.php?id=<?php echo $row['video_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}?>">
														<?=$row['video_title'];?>
														  </a>
                                                          
                                                          
														</td>
										                  <td align="left"><?  $x="../../../uploads/vimages/".$row['video_image'];?>
                   							<img class="image_gap" src="<? echo $x; ?>"  width="100" height="60" alt="" border="0" /></td>
										                  <td align="left"><?=$row['youtube_url'];?></td>
                                                          
                                                           
										                  <td align="left">
                                                         
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['video_id']?>&id=<?=$row['video_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?php 
														  ...
														  echo"Inactive";
														  ?></a>
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id=<?php echo $row['video_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"></a></td> 
									                  </tr>
													  <?php }
													  		else if($val == '1')
															{
													  ...
													  <tr class="row_color">
														 ...
														 <?=$row['video_title']?>
										                  ...
										                  <td align="left"><?=$row['youtube_url'];?></td>
										                  <td align="left">
                                     <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['video_id']?>&id=<?=$row['video_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"><?php 
														  if($row['status']=="active")
														  echo"Active";
														  else
														  echo"Inactive";
