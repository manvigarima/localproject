...
$al=".";
	...
	//}
	$keys=new keys();
	$totalEntries=$keys->totalNoOfkeys();
	...
	$rec = $keys->display_keys($page,$order);	
/*	echo"<pre>";
	print_r($rec);
*/	
...
<script language="javascript">
	...
	{
	   //alert("hi");
	   var num=document.keys.elements.length;
	   for(i=0;i<num;i++)
	   {
			tmp=document.keys.elements[i];
	...
	function send_select()
	{
		...
		//alert("alert");
		document.keys.send.value=value12;
	...
	function doselect1()
	{
		...
		document.keys.delet.value=val12;
		document.keys.submit();
		return true;
	}
		 function export1()
	{
		...
		location.href='export_keys.php?exprt='+val12;
		return true;
...
<body>
	<form name="pageSelectionForm" id="pageSelectionForm" method="post" action="viewkeys.php">
    ...
    <?php  include '../left.php'; ?>
		...
		<div id="content_wrapper" style="margin-right:30px;"> 
			...
			<ul class="first_level_tab"> 
				...
				<li> 
					<a href="<?php echo $admin_path;?>schools/new.php"> 
						 Generate Keys
					</a> 
				</li> 
                  <li> 
					<a href="<?php echo $admin_path;?>schools/viewkeys.php" class="active"> 
						 View Keys
					</a> 
				</li>
			...
			<div style="border:1px solid #ccc;"  > 
				...
				<div class="content nomargin"  > 
                  ...
                  <form name="keys" method="post" action="action.php?al=<? echo $_REQUEST['al'];?>&page=<? echo $page; ?>">
						 ...
						 <tr><td width="100%" colspan="5"><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></td></tr>
											<tr>
                                             ...
                                             <?php
								               ...
								               <input type="submit" name="sub3" value="Delete keys(s)" class="button_light" onclick="doselect1(); return false;" />
											...
											<tr>
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
                  									  ...
                  									  <tr class="row_1">
										                    ...
										                    <th width="30%" align="left" ><a href="viewkeys.php?al=<?php echo $al;?>&order=<?php if($order==0){?>1<?php }else{?>0<?php }?>" class="tablehead"><span class="vote">  Key Number</span></a>&nbsp;<?php if($order==0) {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
									                    <th width="30%" align="left" ><a href="viewkeys.php?al=<?php echo $al;?>&order=<?php if($order==2){?>3<?php }else{?>2<?php }?>" class="tablehead">School Name</a>&nbsp;<?php if($order=='2') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '3'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>	
                                                        <th width="20%" align="left" ><a href="viewkeys.php?al=<?php echo $al;?>&order=<?php if($order==4){?>5<?php }else{?>4<?php }?>" class="tablehead"><strong>Status</strong></a>&nbsp;<?php if($order == '4') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '5'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['keyId'];?>" value="642" class="input2" /></td>
													    <td align="left">
													      <? echo $row['keyNumber'] ;?>
													    </td>
										                  <td align="left"><? 
											 ...
											 $row['school_id'];
											  echo $schools->get_school_name($row['school_id']);
											  ...
											  echo $row['faculty'] ;?></td>
                                                          ...
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['keyId']?>&id=<?=$row['keyId']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <? echo $row['status']; ?></td></a>
                                                        <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id=<?php echo $row['keyId']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"></a></td> 
									                  </tr>
													  <?php }
													  		else if($val == '1')
															{
											 ...
											 $row['school_id'];
											  ...
											  echo $row['faculty'] ;?></td>
										                  ...
										                  <td align="left">
                                     <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['keyId']?>&id=<?=$row['keyId']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"><? echo $row['status']; ?></a></td> 
									                  </tr>
													 <?php } 
													 }
													 ?>
