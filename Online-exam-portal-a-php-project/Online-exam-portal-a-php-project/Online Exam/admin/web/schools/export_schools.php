...
//echo $sql;
	...
	if(isset($_REQUEST['exprt']) && !empty($_REQUEST['exprt']))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$sql = "SELECT * FROM ise_schools WHERE school_id ='".$val[$i]."'";
	...
	if(is_array($users))
		{
				...
				<tr  bgcolor='#72ACF3'>
				<td>Search Criteria: Schools</td> 
				<td>Total Records: ".count($users)."</td></tr>
				<tr><td colspan='2' align='center' style='color:#336699; font-size:16px;'><strong>Schools</strong></td></tr>";
				...
				for($i=0;$i<count($users);$i++)
				{
					...
					<tr><td style=' font-weight:bold;'><strong>School Name:</strong>".ucfirst($users[$i]['school_name'])." </td></tr>
