<?php 
	...
	if(!empty($_POST))
	{
		$count = $ise_news->ise_news_select("count(*) as count","news_title", $_POST['txtnews_title']);
		if(($count == '0')||($count == '1'))
		{
			...
			{	
				...
				if($image=="")
				{
				$tab = array( "news_desc ~!@".$_POST['desc']."", "last_update ~!@ $date","status ~!@ ".$_POST['status']."");
				}
				else
				{
					$tab = array( "news_desc ~!@ ".$_POST['desc']."", "news_pic ~!@ ".$upload."","last_update ~!@ $date", "status ~!@ ".$_POST['status']."");
				}
				$where = "news_id=".$_GET['id']."";
				$val = $ise_news->ise_news_update($tab,$where);
				$page=$_REQUEST['page']; 
				
				$_SESSION['msg'] = "<font size='2' color='#FF0000'>Modified Successfully</font>";
				print "<script>";
		...
		{
			$_SESSION['msg'] = "<font size='2' color='#FF0000'>Class Name Already Existed</font>";
		}
	}
	$row = $ise_news->ise_news_selectall("news_id",$_GET['id']);
?>
...
<script language="javascript">
	function check(){
		if(!confirm('Are you sure you want to modify the news?\n- to modify the news, hit OK\n- otherwise, hit Cancel'))
		return false;
        ...
        <input type="hidden" name="al" id="al" value="<?php echo $_REQUEST['al'];?>" />
									<table width="100%" align="left" border="0" cellpadding="0" cellspacing="0" class="global">
										<tbody>
                                            ...
                                            <tr>
												<td align="center" colspan="6"><h2>Modify News</h2></td>
										  </tr>
											<tr>
											...
											<tr>
												<td colspan="6">
													<table width="100%" border="0" style="line-height:30px;" cellpadding="3" cellspacing="1" class="tborder">
									                  ...
									                  <tr>
										                  <td width="42%" height="24" align="right" >News Title</td>
														  <td width="2%">:</td>
														  <td width="56%">
                                                          <input   type="hidden" value="<?php if(!empty($_POST)){echo $_POST['txtnews_title'];}else{echo $row['news_title'];}?>" name="txtnews_title1" id="txtnews_title1" size="35"/>
                                                          <input  disabled="disabled" type="text" value="<?php if(!empty($_POST)){echo $_POST['txtnews_title'];}else{echo $row['news_title'];}?>" name="txtnews_title" id="txtnews_title" size="35"/></td>
									                  </tr>
													  <tr>
										                  <td width="42%" height="24" align="right" >Description</td>
														  <td width="2%">:</td>
														  <td width="56%"><textarea name="desc" id="desc"><?php if(!empty($_POST)){echo $_POST['desc'];}else{echo stripslashes($row['news_desc']);}?></textarea></td>
										              </tr>
                                                      <tr>
                                                      ...
                                                      <tr>
										                  <td width="42%" height="24" align="right" >Exist Image</td>
														  <td width="2%">:</td>
														  <td width="56%">
                                                          <img src="../../../uploads/news/<?php echo $row['news_pic']?>" height="50" width="50" />
														  </td>
										              </tr>
									                  <tr>
													  ...
													  <tr>
													  	<td colspan="3" align="center" bgcolor="#D8D8F3">
											            	<input type="submit" name="add_new" value="Modify News" class="button" id="add_new">
