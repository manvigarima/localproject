...
$al=".";
	...
	$text=new text();
	$totalEntries=$text->totalNoOftext($al,$_SESSION['school_id']);
	...
	$rec = $text->display_text($page,$order,$al,$_SESSION['school_id']);	
/*	echo"<pre>";
	print_r($rec);
*/	
...
<script language="javascript">
	...
	{
	   //alert("hi");
	   var num=document.text.elements.length;
	   for(i=0;i<num;i++)
	   {
			tmp=document.text.elements[i];
	...
	function addtext()
	{
	if(!confirm('Are you sure you want to add the Scroll Text?\n- to Add the Scroll Text, hit OK\n- otherwise, hit Cancel'))
	return false;
	...
	function send_select()
	{
		...
		//alert("alert");
		document.text.send.value=value12;
		document.text.submit();
		return true;
	}
	function export1()
	{
	 self.location('export_text.php');
	}
	function doselect1()
	{
		dml1=document.text;
		...
		document.text.delet.value=val12;
		document.text.submit();
		return true;
	}
	function addtext()
	 ...
	 function export1()
	{
		...
		location.href='export_text.php?exprt='+val12;
		return true;
		...
		<div id="content_wrapper" style="margin-right:30px;"> 
			...
			<div style="border:1px solid #ccc;"  > 
				...
				<div class="content nomargin"  > 
                  ...
                  <form name="text" method="post" action="action.php?al=<? echo $_REQUEST['al'];?>&page=<? echo $page; ?>">
						...
						<thead>
						 ...
						 <tr><td width="100%" colspan="5"><b><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></b></td></tr>
											<tr >
												...
												?>     
                                                <input type="button" name="sub1" value="Add text" class="button_light"   onclick="return addtext()" />
									               <input type="submit" name="sub3" value="Delete text" class="button_light" onclick="doselect1(); return false;" />
												...
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
                  									  ...
                  									  <tr class="row_1">
										                    ...
										                    <th width="30%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==0){?>1<?php }else{?>0<?php }?>" class="tablehead"><span class="vote">Text</span></a>&nbsp;<?php if($order==0) {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
									                    <th width="30%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==2){?>3<?php }else{?>2<?php }?>" class="tablehead"> Displayed Position</a>&nbsp;<?php if($order=='2') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '3'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>	
													  ...
													  <tr class="row_white">
														  ...
														  <td align="left"><a href="edit.php?id=<?php echo $row['scroll_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."";}?>">
														   <?php echo $row['scroll_text'];?>
														  </a><!--<a href="edit.php?id=<?php //echo $row['scroll_id'];?>"><img class="image_gap" src="<? echo $x; ?>"  width="100" height="60" alt="" border="0" />
                                                          
                                                          
														  </a>--></td>
										                  <td align="left"><?php echo $row['place_displayed'];?></td>
