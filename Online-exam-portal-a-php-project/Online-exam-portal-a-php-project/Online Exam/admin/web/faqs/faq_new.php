<?php 
	session_start();
	include_once '../../../lib/calss_qatar_faqs.php';
	include_once '../../../lib/db.php';
	include_once"../../../lib/admin_check.php";
	admin_login_check('0','Others');
	$qatar_faqs = new qatar_faqs();
	$queries = new Queries();
	if(!empty($_POST))
	{
		$count = $queries->makeselectquery('faqs',"count(*) as count","faq_question", $_POST['txttitle']);
		if($count == '0')
		{
			$date = date('Y-m-d');
			$tab = array("faq_question =".addslashes($_POST['txttitle'])."", "faq_answer =".addslashes($_POST['answer'])."", "create_date=$date","status=".$_POST['state']."","school_id=".$_SESSION['school_id']."");
			$val = $qatar_faqs->qatar_faqs_insert($tab);
			$_SESSION['msg'] = "<font size='2' color='#FF0000'>Added Successfully</font>";
			print "<script>";
		...
		{
			$_SESSION['msg'] = "<font size='3' class = 'linkl'>Question Already Existed</font>";
		}
	}
?>
...
<script language="javascript">
	function check(){
		...
		getForm("new_news");
		if(!IsEmpty("txttitle","Please Enter Question"))return false;
    ...
    <br class="clear"/> 
		...
		<form action="#" method="post" name="new_news" onsubmit="return check()" enctype="multipart/form-data">
									<table width="100%" align="left" border="0" cellpadding="0" cellspacing="0" class="global">
                                      ...
                                      <tr>
												<td align="center" colspan="6"><h2>Add Faq</h2></td>
											</tr>
											<tr>
											...
											<tr>
												<td colspan="6">
													<table width="100%" border="0" style="line-height:30px;" cellpadding="3" cellspacing="1" class="tborder">
									                  ...
									                  <tr>
										                  <td width="42%" height="24" align="right" >Question</td>
													    <td width="2%">:</td>
														  <td width="56%"><input type="text" value="<?php if(!empty($_POST)){echo $_POST['txttitle'];}?>" name="txttitle" id="txttitle" /></td>
										              </tr>
													  <tr>
										                  <td width="42%" height="24" align="right" >Answer</td>
													    <td width="2%">:</td>
														  <td width="56%"><textarea name="answer" id="answer"><?php if(!empty($_POST)){echo $_POST['answer'];}?></textarea></td>
										              </tr>
                            ...
                            <option value="inactive" <?php if((!empty($_POST))&&($_POST['state']=='inactive')){echo "selected";}?> >Inactive</option>
													  ...
													  <tr>
													  	<td colspan="3" align="center" bgcolor="#D8D8F3">
											            	<input type="submit" name="add_new" value="Add Faq" class="button" id="add_new">
