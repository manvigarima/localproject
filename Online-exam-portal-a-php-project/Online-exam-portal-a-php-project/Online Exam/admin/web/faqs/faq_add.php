<?php 
	...
	if(!empty($_POST))
	{
		echo $count = $queries->makeselectquery('ise_faqs',"count(*) as count","faq_question", $_POST['txttitle']);
		if($count == '0')
		{
			//$date = date('Y-m-d');
			$tab = array("faq_question =".addslashes($_POST['txttitle'])."", "faq_answer =".addslashes($_POST['answer'])."", "status=".$_POST['state']."");
			$val = $ise_faqs->ise_faqs_insert($tab);
			$_SESSION['msg'] = "<div class='success'>Added Successfully</div>";
			print "<script>";
...
<script language="javascript">
	function check(){
		...
		if(!IsEmpty("answer","Please Enter Answer"))return false;
                            ...
                            <option value="inactive" <?php if((!empty($_POST))&&($_POST['state']=='inactive')){echo "selected";}?> >Inactive</option>
													  ...
													  <tr>
													  	<td colspan="3" align="center" >
											            	<input type="submit" name="add_new" value="Add Faq" class="button_light" id="add_new" style="cursor:pointer">
