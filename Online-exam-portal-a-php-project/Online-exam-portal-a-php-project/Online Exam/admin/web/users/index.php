...
$al=".";
	...
	//echo $_REQUEST['school'];
	$totalEntries=$user->totalNoOfusercount($al,$school_id);
	...
	if(!empty($_REQUEST['pageNumber'])){
		if($_REQUEST['pageNumber']>$maxPages)
			$_REQUEST['pageNumber']=$maxPages;
	...
	$rec = $user->display_users($page,$al,$order,$school_id);	
/*	echo"<pre>";
	print_r($rec);
*/	
...
<script language="javascript">
	...
	return false;
	javascript:location.href=('user_new_bulk.php');
	}
	...
	//alert("asaedsd");
		...
		<?php if($_REQUEST['school']!='')
		{
		?>
		location.href='export_users.php?exprt='+val12+'&school_id='+<?php echo $_REQUEST['school'];?>;
		<?php }else
		{?>
		location.href='export_users.php?exprt='+val12;
		<?php }?>
		return true;
    ...
    <?php  include '../left.php'; ?>
		...
		<div id="content_wrapper" style="margin-right:30px;"> 
			...
			<ul class="first_level_tab"> 
				...
				<li> 
					<a href="<?php echo $admin_path;?>users/user_new.php" class="active1" onclick="addUser(); return false;" > 
						Add User
					</a> 
				</li> 
				...
				<div class="content nomargin"  > 
					...
					<table class="global" width="100%"  cellpadding="0" cellspacing="0"> 
						<thead>
						 ...
						 <tr><td width="100%" colspan="5"><?php 
						 if(!empty($_GET['exists']) && $_GET['exists']!='')
						 {
						      echo "<font size='2' color='#0099FF'>".$_GET['exists']." Already Exists</font>";
							  if(!empty($_SESSION['msg']))unset($_SESSION['msg']);
						 }
						 else
						 {
						 	echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);
						 }
						 ?></td></tr>
											<tr>
												...
												?>
                                                <label><font class="tablehead">SEARCH BY SCHOOL:</font></label><?php if(!empty($_REQUEST)){echo $settings->get_admin_sel_schools($_REQUEST['school']);}else{echo $ise_settings->get_admin_schools();}?>  
                                                <!--	<input type="submit" name="sub3" value="Add User(s)" class="button_light" />-->
                                                	<input type="submit" name="sub3" value="Delete User(s)" class="button_light" onclick="doselect1(); return false;" style="cursor:pointer"/>
												...
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
                  									  ...
                  									  <tr class="row_1">
										                    ...
										                    <th width="22%" align="left" ><span class="vote">Name</span>&nbsp;</th>
                                                            <th width="22%" align="left" ><span class="vote">User Name</span>&nbsp;</th>
									                    <th width="22%" align="left" >School&nbsp;</th>	
									                    <th width="22%" align="left" ><strong>Create Date </strong>&nbsp;<?php if($order == '4') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '5'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th> 
									                    <th width="10%" align="left" >Status&nbsp;<?php if($order == '6') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '7'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
										              </tr>
													  <?php 
														if($rec !='1')
														{
									                  ...
									                  <?php 
															...
															while($row = $objSql1->fetchRow($rec))
															{
																...
																$arr=$user->get_email($row['studentid']);
																															
																
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['user_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="user_modify.php?id=<?php echo $row['user_id'];?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."";}?>&page=<?php echo $page; ?>">
														  <?php echo ucfirst($row['user_fname'])." ".ucfirst($row['user_lname']);?></a></td>
                                                          <td align="left"><?php echo $row['user_email'];?></td>
										                  <td align="left"><?php echo $settings->get_school_names($row['school']);?></td>
										                  <td align="left"><?php echo  $row['create_date'];?></td>
										                  <td align="left">
                                                        
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['user_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?php 
                 ...
                 <input type="hidden" name="school" id="school" /></form>
				<br class="clear"/>
				</div> 
				</div></div>
