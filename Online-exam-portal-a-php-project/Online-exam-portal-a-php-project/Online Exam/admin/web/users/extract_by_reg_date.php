...
$date=date('Y-m-d');
if($_REQUEST['frm_date']>$date || $_REQUEST['to_date']>$date)
{
	print("<script>");
	print("alert('Future Dates are not Selected');");
...
{
		$sql="select distinct(enm_user.user_id), enm_user.user_fname,enm_user.user_lname,enm_user.user_email,enm_user.user_gender,enm_user.mobile_no,enm_country.country_name from enm_user,enm_country,enm_cart where enm_user.create_date >= '".$_REQUEST['frm_date']."' AND enm_user.create_date <='".$_REQUEST['to_date']."'  and enm_user.status='active' and enm_user.country=enm_country.country_id";
		$users=$objSql1->executeSql($sql);
		
		if(is_array($users))
		{
		
		$filename ="users_report_byDate".time().".doc";
		$title = "";
