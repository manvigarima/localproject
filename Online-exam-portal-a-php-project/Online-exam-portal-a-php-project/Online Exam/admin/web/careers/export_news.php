...
//echo $sql;
	...
	if(isset($_REQUEST['exprt']) && !empty($_REQUEST['exprt']))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$sql = "SELECT * FROM career WHERE news_id='".$val[$i]."' ";
...
$title = "";
		...
		<tr bgcolor='#72ACF3'><td colspan='3' height='30' align='center' style='font-size:14px;'><b>Qatarsmarteducation Careers Report</b></td></tr>
		<tr  bgcolor='#72ACF3'>
		<td>Search Criteria: Careers</td> 
		<td>Total Records: ".count($users)."</td><td>Date:  ".date('Y-m-d')."</td></tr>
		<tr><td colspan='3' align='center' style='color:#336699; font-size:16px;'><strong>Careers</strong></td></tr>";
		...
		for($i=0;$i<count($users);$i++)
		{
			...
			<tr><td style='font-size:14px; color:#009966; font-weight:bold;'>".ucfirst($users[$i]['career_name'])." </td></tr>
			<tr><td><strong>Status : ".$users[$i]['status']."</strong> </td></tr>
			<tr><td style='padding-left:5px;' align='justify'>".nl2br(ucfirst($users[$i]['career_desc']))." </td></tr>
			<tr><td>&nbsp;</td></tr>
			</table>
			</td></tr>";
		}
