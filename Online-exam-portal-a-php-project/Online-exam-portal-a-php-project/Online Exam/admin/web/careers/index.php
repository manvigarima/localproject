<?php 
	...
	$totalEntries=$careers->totalNoOfcareers($al);
	...
	$rec = $careers->display_careers($page,$al,$order);
...
<script language="javascript">
	...
	function addNews()
	{
		if(!confirm('Are you sure you want to add the career?\n- to Add the career, hit OK\n- otherwise, hit Cancel'))
		return false;
	
	javascript:location.href=('career_new.php');
	}
...
<body>
    ...
    <br class="clear"/> 
		...
		<div id="content_wrapper" style="margin-right:30px;"> 
			...
			<form name="newsSelectForm" method="post" action="careers_action.php?al=<? echo $_REQUEST['al'];?>&page=<? echo $page; ?>">
									<table width="100%" align="left" border="0" cellpadding="0" cellspacing="0" class="global">
										<tbody>
											...
											<tr>
												...
												?>    
													<input type="button" name="sub1" value="Add Career" class="button_light" onclick="return addNews()" />
									                <input type="submit" name="sub3" value="Delete Careers" class="button_light" onclick="doselect1(); return false;" />
												...
												<td colspan="5">
													<table width="100%" border="0" cellpadding="3" cellspacing="1" class="tborder">
                  										...
                  										<tr class="row_1"><?php $_GET['or']?>
										                    ...
										                    <th width="40%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if(empty($_GET['order'])){?>1<?php }elseif($_GET['order'] == '1'){?>0<?php }?>" class="tablehead">Career Title</a>&nbsp;<?php if($_GET["order"]=='0') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($_GET["order"] == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['career_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="careers_modify.php?id=<?php echo $row['career_id'];?>&pageNumber=<?php echo $_REQUEST['pageNumber'];?>&al=<?php echo $_REQUEST['al'];?>">
														  <?php echo ucfirst($row['career_name']);?></a></td>
                                                          <td align="left"><?php echo $row['create_date'];?></td>
										                  <td align="left">
                                                          <a href="careers_action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";} ?>id=<?php echo $row['career_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?php echo  $row['status'];?></a>
                                                          
                                                          
                                                        </td> 
                                                          <td  height="24" align="left" ><a href="view_careers.php?id=<?=$row['career_id'] ?>&pageNumber=<?=$_REQUEST['pageNumber']?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."";} ?>">
													  ...
													  <tr class="row_color">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['career_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="careers_modify.php?id=<?php echo $row['career_id'];?>&pageNumber=<?php echo $_REQUEST['pageNumber'];?>&al=<?php echo $_REQUEST['al'];?>"><?php echo ucfirst($row['career_name']);?></a></td>
                                                          <td align="left"><?php echo $row['create_date'];?></td>
										                  <td align="left"><a href="careers_action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";} ?>id=<?php echo $row['career_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"><?php echo  $row['status'];?></a></td> 
                                                          <td  height="24" align="left" ><a href="view_careers.php?id=<?=$row['career_id'] ?>&pageNumber=<?=$_REQUEST['pageNumber']?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."";} ?>">
