...
//echo $sql;
	...
	if(isset($_REQUEST['exprt']) && !empty($_REQUEST['exprt']))
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$sql = "SELECT * FROM ise_country WHERE country_id='".$val[$i]."' ";
	...
	if(is_array($users))
		{
				...
				$filename ="country_report".time().".doc";
				$title = "";
				...
				<tr  bgcolor='#72ACF3'>
				<td>Search Criteria: Country</td> 
				<td>Total Records: ".count($users)."</td></tr>
				<tr><td colspan='2' align='center' style='color:#336699; font-size:16px;'><strong>Country</strong></td></tr>";
				...
				for($i=0;$i<count($users);$i++)
				{
					...
					<tr><td style='font-size:14px; color:#009966;'>Country:<b>".ucfirst($users[$i]['country_name'])." </b></td></tr>
					 ";
