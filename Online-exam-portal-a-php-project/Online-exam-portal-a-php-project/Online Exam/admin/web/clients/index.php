...
$al=".";
	...
	$ads=new ads();
	$totalEntries=$ads->totalNoOfads($al,$_SESSION['school_id']);
	...
	$rec = $ads->display_ads($page,$order,$al,$_SESSION['school_id']);	
/*	echo"<pre>";
	print_r($rec);
*/	
...
<script language="javascript">
	 ...
	 function export1()
	{
		...
		location.href='export_clients.php?exprt='+val12;
		return true;
						...
						<thead>
						 ...
						 <tr><td width="100%" colspan="5"><b><font color="#ff0000"><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></font></b></td></tr>
											<tr>
												...
												?>     
                                                <input type="button" name="sub1" value="Add clients" class="button_light"   onclick="return addclients()" />
									               <input type="submit" name="sub3" value="Delete clients" class="button_light" onclick="doselect1(); return false;" />
											...
											<tr>
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
									                    ...
									                    <th width="20%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order==4){?>5<?php }else{?>4<?php }?>" class="tablehead"><strong>Displayed Page </strong></a>&nbsp;<?php if($order == '4') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '5'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th> 
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['ad_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="edit.php?id=<?php echo $row['ad_id'];?>">
														    <? $x="../../../uploads/clients/".$row['ad_path']; ?>
														  </a><a href="edit.php?id=<?php echo $row['ad_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."";} ?>"><img class="image_gap" src="<? echo $x; ?>"  width="60" height="60" alt="" border="0" />
                                                          
                                                          
														  </a></td>
										                  <td align="left"><?php echo $row['ad_link'];?></td>
										                  <td align="left"><?php echo  $row['page_displayed'];?></td>
										                  <td align="left">
                                                         
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['ad_id']?>&id=<?=$row['ad_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?php 
														  if($row['status']==1)
														  echo"Active";
														  else
														  echo"Inactive";
														  ?></a>
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id=<?php echo $row['ad_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"></a></td> 
									                  </tr>
													  <?php }
													  		else if($val == '1')
															{
													  ...
													  <tr class="row_color">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['ad_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="edit.php?id=<?php echo $row['ad_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."";} ?>">
														  <?  $x="../../../uploads/clients/".$row['ad_path']; ?>
														 <img class="image_gap" src="<? echo $x; ?>"  width="60" height="60" alt="" border="0" /></a></td>
										                  <td align="left"><?php echo $row['ad_link'];?></td>
										                  <td align="left"><?php echo  $row['page_displayed'];?></td>
										                  <td align="left">
                                     <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['ad_id']?>&id=<?=$row['ad_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"><?php 
														  if($row['status']==1)
														  echo"Active";
														  else
														  echo"Inactive";
