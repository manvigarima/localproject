<?php 
	...
	//{
		//$al = $_REQUEST['al'];
	//}
	$grp_posts=new ise_groups();
	$totalEntries=$grp_posts->totalNoOfgrp_posts();
	...
	$rec = $grp_posts->display_grp_posts_all($_REQUEST['id']);	
...
<script language="javascript">
	...
	{
	   //alert("hi");
	   var num=document.grp_posts.elements.length;
	   //alert(num);
	   for(i=0;i<num;i++)
	   {
			tmp=document.grp_posts.elements[i];
	...
	function send_select()
	{
		...
		//alert("alert");
		document.grp_posts.send.value=value12;
		document.grp_posts.submit();
		return true;
	}
	function export1()
	{
	...
	function doselect1()
	{
		...
		document.grp_posts.delet.value=val12;
		document.grp_posts.submit();
		return true;
	}
...
function popup(url)
			 ...
			 function export1()
	{
		...
		location.href='export_posts.php?exprt='+val12+'&id='+<?php echo $_REQUEST['id'];?>;
		return true;
...
<body>
	<form name="pageSelectionForm" id="pageSelectionForm" method="post" action="view_posts.php">
		...
		<ul class="first_level_tab"> 
			...
			<div style="border:1px solid #ccc;"  > 
				...
				<div class="content nomargin"  > 
                  ...
                  <form name="grp_posts" method="post" action="action_posts.php?al=<? echo $_REQUEST['al'];?>&page=<? echo $page; ?>&group_id=<? echo $_REQUEST['id']; ?>">
						 ...
						 <tr><td width="100%" colspan="5"><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></td></tr>
											<tr>
                                                ...
                                                <!--<input type="button" name="sub1" value="Add group" class="button"   onclick="return addgroup()" />-->
									               <input type="submit" name="sub3" value="Delete posts" class="button_light" onclick="doselect1(); return false;" style="cursor:pointer" />
											...
											<tr>
											  <td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
									                    ...
									                    <th width="30%" align="left" ><a href="view_posts.php?id=<?php echo $_REQUEST['id'];?>&al=<?php echo $al;?>&order=<?php if($order==0){?>1<?php }else{?>0<?php }?>" class="tablehead"><span class="vote">Comment</span></a>&nbsp;<?php if($order==0) {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
                                                        <th width="10%" align="left" ><a href="view_posts.php?id=<?php echo $_REQUEST['id'];?>&al=<?php echo $al;?>&order=<?php if($order==6){?>7<?php }else{?>6<?php }?>" class="tablehead">Status</a>&nbsp;<?php if($order == '6') {?><img src="../images/up1.GIF" width="13" height="13" /><?php }else if($order == '7'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
										              </tr>
													  <?php 
														if(is_array($rec))
														{
													  ...
													  ?>
													  <tr class="row_white">
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['post_id'];?>" value="642" class="input2" /></td>
														  <td align="left">
														  <?php $val = $ise_users->ise_users_select('user_fname','user_id',$row['user_id']);print_r($val);?>
                                                     ...
                                                     <?=$row['message']?></td>
                                                          ...
                                                          <a href="action_posts.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>&id=<?=$row['post_id']?>&group_id=<?php echo $row['group_id'];?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?=$row['status'];?></a>
                                                         </td>
