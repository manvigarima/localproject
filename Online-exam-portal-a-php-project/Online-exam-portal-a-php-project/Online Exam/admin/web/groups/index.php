<?php 
	...
	//$ise_group_events = new ise_group_events();
...
$al=".";
	...
	$ise_group=new ise_groups();
	$totalEntries=$ise_group->totalNoOfgroup($al,$sid);
	...
	//echo $sid;
	$rec = $ise_group->display_group($page,$order,$al,$sid);	
...
function change_school(sid)
{
		 ...
		 function export1()
	{
		...
		location.href='export_groups.php?exprt='+val12;
		return true;
...
<body>
    ...
    <?php  include '../left.php'; ?>
		...
		<ul class="first_level_tab"> 
				...
				<li> 
					<a href="<?php echo $admin_path;?>groups/new.php" class="active1"> 
						 Add Groups
					</a> 
				</li> 
						...
						<thead>
						 ...
						 <tr><td width="100%" colspan="5"><b><?php echo $_SESSION['msg']; if(!empty($_SESSION['msg']))unset($_SESSION['msg']);?></b></td></tr>
											<tr>
												...
												?>   
                                                <label><font class="tablehead">SEARCH BY SCHOOL:</font></label><?php if(!empty($_REQUEST)){echo $ise_settings->get_admin_sel_schools($_REQUEST['school']);}else{echo $ise_settings->get_admin_schools();}?>  
                                                <!--<input type="button" name="sub1" value="Add group" class="button_light"   onclick="return addgroup()" style="cursor:pointer"/>-->
									               <input type="submit" name="sub3" value="Delete group(s)" class="button_light" onclick="doselect1(); return false;" style="cursor:pointer" />
												...
												<td colspan="5">
													<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tborder">
                  									  ...
                  									  <tr class="row_1">
										                    ...
										                    <th width="15%" align="left" ><a href="index.php?al=<?php echo $al;?>&order=<?php if($order=='0'){?>1<?php }elseif($_GET['order'] == '1'){?>10<?php }elseif($_GET['order'] == '10'){?>1<?php }?>&school=<?php if($sid!='.') echo $sid;?>" class="tablehead"><span class="vote">Group Name</span></a>&nbsp;<?php if($order==0) {?><img src="../images/up.GIF" width="13" height="13" /><?php }else if($order == '1'){?><img src="../images/down1.GIF" width="13" height="13" /><?php }?></th>
									                    <th width="15%" align="left" class="tablehead"><span class="vote">School Name</span></th>
                                                        ...
                                                        <th width="15%" align="left"  class="tablehead">Total Members</th>
                                                       
                                                        <th width="15%" align="left" class="tablehead">Group Posts</th>
										              </tr>
													  <?php 
														if($rec !='1')
														{
													  ...
													  ?>
										                  ...
										                  <td height="24" align="left" ><input  type="checkbox" name="Mid[]" id="<?php echo $row['group_id'];?>" value="642" class="input2" /></td>
														  <td align="left"><a href="edit.php?id=<?php echo $row['group_id'];?>&page=<?php echo $page; ?>&<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."";}?>">
														    <?=$row['group_name'];?>
														  </a></td>
                                                          
                                                          <td align="left">
														    <?php $school_name=$ise_settings->get_school_name($row['school_id']); if($school_name!='')echo $school_name; else echo "NOT AVAILABLE";?>
														  </a></td>
														
										               
										                  <td align="left">
                                                         
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id_1=<?=$row['group_id']?>&id=<?=$row['group_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>">
														  <?=$row['status'];?></a>
                                                          <a href="action.php?<?php if(!empty($_REQUEST['al'])){echo "al=".$_REQUEST['al']."&";}if(!empty($_REQUEST['or'])){echo "or=".$_REQUEST['or']."&";}?>id=<?php echo $row['group_id']?>&state=<?php echo $row['status'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"></a></td>
                                                          
                                                           <td align="left"><?php echo $ise_groups->ise_groups_memb_select("count(*) as count","group_id", $row['group_id']);?> <a href="view_mem.php?id=<?=$row['group_id']?>&name=<?=$row['group_name']?>"><img src="../../images/view_icon.png" height="20" border="0" title="View Group Members" width="20" /></a> </td>
                                                            
                                                            
                                                              <td align="left"><?=$ise_group_members->ise_group_post_select("count(*) as count","group_id", $row['group_id']);?><a href="view_posts.php?id=<?php echo $row['group_id'];?>&page=<?php echo $page; ?>&order=<?php echo $order;?>"><img src="../../images/view_icon.png" height="20" border="0" title="View Announcements" width="20" /></a></td>
													...
													$val++;  		
													 ...
													 <tr class="row_white"><td align="center" colspan="9" style="color:#FF0000;">No records Found.</td></tr>
													 <?php } ?>
											  </table>											  </td>
											</tr>
                                            	<tr>
