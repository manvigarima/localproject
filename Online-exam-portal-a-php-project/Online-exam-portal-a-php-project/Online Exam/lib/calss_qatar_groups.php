...
class qatar_groups{
    ...
    function __construct(){
	  ...
	  $this->table_mem='group_members';
      $this->query=new Queries();
	}
	function qatar_groups_insert($array){
	...
	function qatar_groups_update($array,$where){
	   return $this->query->makeupdatequery($this->table,$array,$where);	
	}
	...
	function qatar_groups_selectall($field,$value){
		...
		function get_active_othergroups_limit($school_id,$userid)
			{
			 ...
			 $sql = "SELECT * FROM groups where (school_id=$school_id and group_id not in(select distinct(group_id) from group_members where user_id='$userid'))
	...
	function get_active_mygroups_limit($school_id,$userid)
			{
			$limit=5;
			 $sql = "SELECT * FROM groups where status='active' and school_id=$school_id and group_id in(select distinct(group_id) from group_members where user_id='$userid')
	...
	function get_group_members($schoolid,$groupid)
	{
	 ...
	 $sql="select first_name,photo from sis_students where studentid in(select distinct(user_id) from group_members where group_id=$groupid)";
	$objSql = new SqlClass();
			$res = $objSql->executeSql($sql);
			return $res;
	}
	...
	function get_active_groups($school_id)		{
			$limit=5;
			$sql = "SELECT * FROM groups where status='active' and school_id=$school_id ORDER BY group_id ASC";
	...
	function get_active_othergroups($school_id,$userid)
			{
			$limit=6;
			 $sql = "SELECT * FROM groups where (school_id=$school_id and group_id not in(select distinct(group_id) from group_members where user_id='$userid')) or school_id=$school_id and status='inactive' and group_id in(select distinct(group_id) from group_members where user_id='$userid')
			 ORDER BY group_id ASC ";
		$pagination_qatar = new pagination_key();
		$pagination_qatar->createPaging($sql,$limit);
		while($row=mysql_fetch_object($pagination_qatar->resultpage)){
