...
<p><?php echo sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment','health-center-lite' ), site_url( 'wp-login.php' ) . '?redirect_to=' .  urlencode(get_permalink())); ?></p>
<?php else : ?>
<div class="hc_comment_form_section">
  <?php  
    ...
    'logged_in_as' => '<p class="logged-in-as">' . __("Logged in as",'health-center-lite').' '.'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'. wp_logout_url( get_permalink() ).'" title="'.__("Logout of this account",'health-center-lite').'">'.' '.__("Logout",'health-center-lite').'</a>' . '</p>',
    'id_submit'=> 'hc_btn',
